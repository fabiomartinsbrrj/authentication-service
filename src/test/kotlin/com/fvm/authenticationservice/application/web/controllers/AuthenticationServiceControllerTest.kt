...
internal class AuthenticationServiceControllerTest {
    ...
    private val passwordServiceController = AuthenticationServiceController(passwordService)

    @Test
    fun `given a valid json request must perform a password service with success`() {

        every { context.bodyValidator<AuthenticationRequest>().get() } returns AuthenticationRequest("AbTp9!foo")
